...
Ajax follows an asynchronous pattern, which makes Ajax requests _non-blocking_.
...
_have_ to). We wait until the beep tells us it's done. Checking every 5 seconds
is called _polling_, and it's a lot less efficient than waiting for the beep,
which is our _callback_.
